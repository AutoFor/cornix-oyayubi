> **Note / 注意**: Nordic SoftDevice およびブートローダのバイナリ（`*.hex` / `s140_*.uf2`）は
> 再配布条件があるため、この公開リポジトリには**同梱していません**。
> 下記の Official repository から HEX を各自ダウンロードし、この README のコマンドで
> `s140_restore.uf2` を生成してください（`uf2conv.py` は同梱）。

### 软设备恢复说明（中文）

由于原RMK固件移除了SoftDevice，在刷入ZMK固件之前，必须先恢复SoftDevice：

1. **先用SoftDevice恢复包刷入**（`s140_restore.uf2` — 用下面的命令自行生成）
