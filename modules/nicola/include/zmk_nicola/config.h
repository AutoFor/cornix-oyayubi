...
struct nc_settings {
    ...
    uint32_t lthumb_tap; /* 左親指シフト単独タップ時の送出キーコード */
    uint32_t rthumb_tap; /* 右親指シフト単独タップ時の送出キーコード */
};

void nc_cfg_get(struct nc_settings *out);

/* key: "timeout" / "range" / "mode" / "cont" / "log" / "lthumb" / "rthumb"。
 * 成功=0、未対応キー=-EINVAL。
 * 値は即時反映され、フラッシュに永続化される(再起動後も維持)。
 * lthumb/rthumb は同時打鍵の判定対象キー自体ではなく、単独タップ時の送出キーコードのみを変更する
 * (判定対象キーの物理位置はkeymapのbinding位置=ZMK Studioで変更可能)。 */
